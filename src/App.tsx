...
function App() {
  ...
  const familySlides = useMemo(() => familyMeta.map((meta) => {
    const members = perfumes.filter((perfume) => getMacroFamily(perfume.family) === meta.name);
    const representative = members[0];
    return { ...meta, count: members.length, image: representative?.image, note: representative?.family ?? meta.line };
  }), [perfumes]);
  const visiblePerfumes = activeFamily === 'Todos'
    ? perfumes
    : macroFamilyNames.includes(activeFamily)
      ? perfumes.filter((perfume) => getMacroFamily(perfume.family) === activeFamily)
      : perfumes.filter((perfume) => perfume.family === activeFamily);
  ...
  const sendWhatsApp = () => {
    ...
    window.open(`https://wa.me/5491124578934?text=${message}`, '_blank', 'noopener,noreferrer');
  };
    ...
    <div className="min-h-screen bg-[#0b0b0a] text-[#f2eee7] selection:bg-[#c99558] selection:text-black">
      ...
      <main>
        <section id="inicio" className="relative flex min-h-[640px] items-end overflow-hidden bg-[#080909] px-6 pb-8 pt-28 md:min-h-screen md:px-12 md:pb-20 md:pt-32">
          <video className="absolute inset-0 hidden h-full w-full object-cover object-[70%_center] md:block" autoPlay={!prefersReducedMotion} muted loop={!prefersReducedMotion} playsInline poster="/images/perfumes/art-of-universe.png" aria-hidden="true"><source src="https://res.cloudinary.com/u5z5trw7/video/upload/v1786922221/Perfume_bottle_in_dark_water_202608162003.mp4" type="video/mp4" /></video>
          <video className="absolute inset-0 h-full w-full object-cover object-center md:hidden" autoPlay={!prefersReducedMotion} muted loop={!prefersReducedMotion} playsInline poster="/images/perfumes/art-of-universe.png" aria-hidden="true"><source src="https://res.cloudinary.com/u5z5trw7/video/upload/v1787003949/Perfume_bottle_in_dark_water_202608171858.mp4" type="video/mp4" /></video>
        ...
        <section id="coleccion" className="bg-[#e9e5dd] px-5 py-20 text-[#151412] md:px-12 md:py-28">
          <div className="mx-auto max-w-[1440px]"><div className="mb-14 flex flex-col justify-between gap-8 md:flex-row md:items-end"><div><p className="mb-4 text-[10px] uppercase tracking-[0.3em] text-[#96724b]">La colección</p><h2 className="max-w-xl font-serif text-5xl leading-[.92] tracking-[-0.06em] md:text-7xl">Elegí la nota<br /><i className="font-light">que habla de vos.</i></h2></div><p className="max-w-xs text-sm leading-6 text-black/55">Seis perfumes, seis maneras de dejar huella. Diseñados para acompañarte, no para pasar desapercibidos.</p></div>
            <div className="mb-12 flex gap-2 overflow-x-auto pb-2">{families.map((family) => <button key={family} onClick={() => setActiveFamilyAndResetPage(family)} className={`whitespace-nowrap rounded-full border px-4 py-2 text-[10px] uppercase tracking-[0.17em] transition ${activeFamily === family ? 'border-[#151412] bg-[#151412] text-white' : 'border-black/20 text-black/55 hover:border-black/60'}`}>{family}</button>)}</div>
            ...
            {catalogTotalPages > 1 && <div className="mt-12 flex items-center justify-center gap-6">
              <button onClick={() => setCatalogPage((page) => Math.max(1, page - 1))} disabled={catalogPage === 1} className="text-[10px] uppercase tracking-[0.2em] text-black/55 transition hover:text-black disabled:cursor-not-allowed disabled:opacity-30">Anterior</button>
              <div className="flex items-center gap-2">{Array.from({ length: catalogTotalPages }, (_, i) => i + 1).map((page) => <button key={page} onClick={() => setCatalogPage(page)} aria-label={`Pagina ${page}`} className={`h-2 w-2 rounded-full transition ${page === catalogPage ? 'bg-[#151412]' : 'bg-black/20 hover:bg-black/40'}`} />)}</div>
              <button onClick={() => setCatalogPage((page) => Math.min(catalogTotalPages, page + 1))} disabled={catalogPage === catalogTotalPages} className="text-[10px] uppercase tracking-[0.2em] text-black/55 transition hover:text-black disabled:cursor-not-allowed disabled:opacity-30">Siguiente</button>
            </div>}
          </div>
        </section>

        <section className="bg-[#0b0b0a] px-5 py-20 md:px-12 md:py-28"><div className="mx-auto grid max-w-[1440px] gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div><p className="mb-4 text-[10px] uppercase tracking-[0.3em] text-[#c99558]">Encontrá tu familia</p><h2 className="font-serif text-5xl leading-[.92] tracking-[-0.06em] md:text-7xl">Una emoción<br /><i className="text-[#c99558]">en cada nota.</i></h2><p className="mt-8 max-w-xs text-sm leading-6 text-white/45">Cada familia cambia de voz, de textura y de energía. Dejá que el próximo aroma te encuentre.</p></div><div className="grid grid-cols-1 gap-px overflow-hidden border border-white/10 bg-white/10 sm:grid-cols-2">{familySlides.map((family, index) => <article key={family.name} onClick={() => { setActiveFamilyAndResetPage(family.name); document.getElementById('coleccion')?.scrollIntoView({ behavior: 'smooth' }); }} className="group relative min-h-[220px] cursor-pointer overflow-hidden p-6 transition sm:min-h-[250px] md:min-h-[300px] md:p-8" style={{ backgroundColor: family.tone }}>{family.image && <img src={family.image} alt="" className="absolute -bottom-8 -right-12 h-[78%] w-[78%] object-contain opacity-55 mix-blend-screen transition duration-500 group-hover:opacity-75 group-hover:scale-105" />}<div className="relative z-10 flex h-full flex-col justify-between"><div className="flex items-center justify-between text-white/45"><Sparkles size={17} /><span className="text-[9px] uppercase tracking-[0.2em]">0{index + 1} / 04</span></div><div><h3 className="font-serif text-2xl md:text-3xl">{family.name}</h3><p className="mt-3 max-w-[180px] text-xs leading-5 text-white/55 sm:max-w-[150px]">{family.line}</p><p className="mt-4 text-[9px] uppercase tracking-[0.16em] text-[#c99558]">{family.count} {family.count === 1 ? 'perfume' : 'perfumes'}</p></div></div></article>)}</div></div></section>
